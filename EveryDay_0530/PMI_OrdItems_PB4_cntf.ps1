...
$localfilename = 'PMI_ORDITEM_PB4_cntf.txt'
$SQLDBName = "DPD_DB"
$table="OrderItems"
$remotefilepath = ""
$Datum = Get-Date
            ...
            ForEach ($rec in $data) 
                {
                    ...
                    $Codentify=$rec.Codentify.Trim()
                     ...
                     if (-not ([string]::IsNullOrEmpty( $REFERENCE))) {                                        
                        ...
                        VALUES  ('$REFERENCE','$ORDTyp','$Material',$Quantity,'$Codentify','')"
...
$SqlQuery = "WITH CTE AS (SELECT [REFERENCE],[Material],[ORDTyp],[Codentify],[Quantity], ROW_NUMBER() OVER (PARTITION BY [REFERENCE],[Material],[ORDTyp],[Codentify],[Quantity] ORDER BY [REFERENCE] Desc) row_num FROM $table) DELETE FROM CTE WHERE row_num > 1 and ORDTyp = 'PB4'"
 $SqlCmd.CommandText = $SqlQuery
 $SqlCmd.CommandText = $SqlQuery
 $SqlCmd.Connection = $SqlConnection
...
#}
 ...
 $MyFileName = "PMI_OrdItems_PD.ps1"
