...
$localfilename = 'PMI_ORDITEM_PD.txt'
$SQLDBName = "DPD_DB"
$table="OrderItems"
$remotefilepath = ""
$Datum = Get-Date
...
$SqlQuery = "WITH CTE AS (SELECT [REFERENCE],[Material],[ORDTyp],[Codentify],[Quantity],[Task], ROW_NUMBER() OVER (PARTITION BY [Task]  ORDER BY [Task] desc) row_num FROM $table) delete FROM CTE WHERE row_num > 1 and left(ORDTyp,2)='PD'"
 $SqlCmd.CommandText = $SqlQuery
 $SqlCmd.Connection = $SqlConnection
...
$LogClause=  $Datum.ToString("dd/MM/yyyy HH:mm:ss")+ "; " + $env:UserDomain + "\" + $env:UserName + "; "+ $env:ComputerName + "; "
$LogString = $LogClause + $file_name + "; Db have " + $Row + " rows; OrdItems_PD Import job done"
Write-output $LogString | Out-File $Logpath -Append                
write-host $LogString   
...
#}
 ...
 $MyFileName = "PMI_ORDITEMs_PD_cntf.ps1"
