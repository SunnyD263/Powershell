...
$EmailCopy = $Emaillog[0] + ";" + $Emaillog[5] + ";" + $Emaillog[3] + ";" + $Emaillog[4] 
...
Invoke-Sqlcmd -ServerInstance $SQLServer -Database $SQLDBName -user $uid -Password $passw -inputfile "\\10.47.17.20\pmi-dbo\SQL_script\Script\Trade_IN_stat.sql"
do
{
  $date = $date.AddDays(0)
...
$SQL = "SELECT [REFERENCE],[Created_Date],[Shipped_Date],[Delivery_Date],[PickUp_Date],[Received_Date],[Parcel_First],[Parcel_Second],[STATUS],[CdfCharger],[CdfHolder],[Final Status] FROM [dbo].[$table] ORDER by [Created_Date] asc"
$SQL1= "SELECT [Description], [Quantity], [Percentage]FROM [DPD_DB].[dbo].[Trade_IN_stat]"
...
if ($Row -ne 0) {
  ...
  if (Test-Path $xlsxFile) {
    ...
    Set-ExcelColumn -Worksheet $ws -Column 8 -NumberFormat '0' -AutoSize
   ...
   $Mail.To = $Email
    $Mail.Cc = $EmailCopy
    $Mail.Subject = $xlsxF
    $Mail.Body = 'Pravidelny report stavu dorucovani TRADE-IN baliku'
