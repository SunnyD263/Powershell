...
$SQL = "SELECT [REFERENCE],[PS_Delv_Date],[PARCELNO],[Notice] FROM [dbo].[$table] ORDER by [PS_Delv_Date] asc"
...
if ( $Row -ne 0) {
  ...
  if (Test-Path $xlsxFile) {
    ...
    $Mail.Cc = $EmailCopy
    $Mail.Subject = $xlsxF
    $Mail.Body = 'Pravidelny report DPD baliku dorucenych do ParcelShopu za poslednich 14dni'
